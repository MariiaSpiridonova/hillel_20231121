...
for row in rows:
    if "Standard" in row:
        sales_standard += float(row[4])
    elif "Second" in row:
        sales_second += float(row[4])
    elif "First" in row:
        sales_first += float(row[4])

result_table = [["Standard", "Second", "First"], [round(sales_standard, 2), round(sales_second, 2), round(sales_first, 2)]]
